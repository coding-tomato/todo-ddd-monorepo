...
test('focus moves into the modal input when the modal opens', async ({ page }) => {
  await page.getByRole('button', { name: /\+ add/i }).click();
...
test('focus returns to + ADD button when modal is closed via Cancel', async ({ page }) => {
  await page.getByRole('button', { name: /\+ add/i }).click();
...
test('focus returns to + ADD button when modal is closed via Escape', async ({ page }) => {
  await page.getByRole('button', { name: /\+ add/i }).click();
...
test('Tab key cycles focus within the modal and does not escape to the page', async ({ page }) => {
  await page.getByRole('button', { name: /\+ add/i }).click();
...
test('+ ADD button opens the modal when activated via keyboard Enter', async ({ page }) => {
...
test('page has an aria-live region for screen reader announcements', async ({ page }) => {
